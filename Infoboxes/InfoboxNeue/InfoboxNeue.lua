...
function methodtable.renderMessage( self, data, noInsert )
	...
	local item = self:renderSection( { content = self:renderItem( { data = data.title, desc = data.desc, row = true } ) }, noInsert )
...
--- @return string html
function methodtable.renderImageOrGallery( self, filenameArg )
	checkType( 'Module:InfoboxNeue.renderImageOrGallery', 1, self, 'table' )

	if filenameArg ~= nil and string.find(filenameArg, ";") then
		return methodtable.renderGallery(self, filenameArg)
	end

	return methodtable.renderImage(self, filenameArg)
end
...
function methodtable.renderInfobox( self, innerHtml, snippetText )
	...
	local function renderSnippet()
		...
		html
			...
			:wikitext( string.format( '%s:', t( 'Quick Facts' ) ) )
			:done()
			:tag( 'div' )
